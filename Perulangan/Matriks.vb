Public Class Matriks
    ...
    Private Sub BUTTON_PROSES_Click(sender As Object, e As EventArgs) Handles BUTTON_PROSES.Click
        Try
            Dim barisMatriks1 As Integer = TEKSBOX_MATRIK1.Lines.Count
            Dim kolomMatriks1 As Integer = TEKSBOX_MATRIK1.Lines.ElementAt(0).Split(" ").Length
            Dim barisMatriks2 As Integer = TEKSBOX_MATRIK1.Lines.Count
            Dim kolomMatriks2 As Integer = TEKSBOX_MATRIK1.Lines.ElementAt(0).Split(" ").Length
        ...
        If (Me.cekKolom) Then
            For baris As Integer = 0 To baris1 - 1
                For kolom As Integer = 0 To kolom1 - 1
                    nilai1(baris, kolom) = TEKSBOX_MATRIK1.Lines.ElementAt(baris).Split(" ").ElementAt(kolom)
                    nilai2(baris, kolom) = TEKSBOX_MATRIK2.Lines.ElementAt(baris).Split(" ").ElementAt(kolom)
                Next
            Next
        End If
    End Sub
    ...
    Private Sub cek(ByVal baris1 As Integer, baris2 As Integer, kolom1 As Integer, kolom2 As Integer)
        ...
        For baris = 0 To baris1 - 2
            If (TEKSBOX_MATRIK1.Lines.ElementAt(baris).Split(" ").Length <> TEKSBOX_MATRIK1.Lines.ElementAt(baris + 1).Split(" ").Length) Then
                tmp = False
            End If
        Next
        For baris = 0 To baris2 - 2
            If (TEKSBOX_MATRIK2.Lines.ElementAt(baris).Split(" ").Length <> TEKSBOX_MATRIK2.Lines.ElementAt(baris + 1).Split(" ").Length) Then
                tmp = False
            End If
        Next
        If (tmp = False) Then
            Me.cekKolom = False
            Convert.ToInt32(TEKSBOX_MATRIK1.Text)
            Convert.ToInt32(TEKSBOX_MATRIK2.Text)
    ...
    Private Sub TEKSBOX_MATRIK1_TextChanged(sender As Object, e As EventArgs) Handles TEKSBOX_MATRIK1.TextChanged
